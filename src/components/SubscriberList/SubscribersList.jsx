...
import CustomTable from '../AdminLibrary/CustomTable/CustomTable';
...
export default function SubscribersList() {
    ...
    function requestData( rowsPerPage = 10, currentPage = 1, productNameField = '' , emailField = '', start_date = sevenDaysAgo, end_date = currentDate, postStatus ) {
        ...
        axios({
            ...
            data: { page: currentPage , row: rowsPerPage, postStatus: postStatus
                        ...
                        <div className="woo-middle-container-wrapper">
                            ...
                            <div className="woo-search-and-multistatus-wrap">
                                <ul className="woo-multistatus-ul">
                                    {
                                        // subscribeStatus.map((status) => (
                                        //     <div onClick={ setPostStatus( status.key ) } className={ postStatus === status.key ? 'status-active' : '' }>
                                        //         { `${status.name} (${subscribersStatus})`}
                                        //     </div>
                                        // ))
                                    }
                                        {/* <li className={`woo-multistatus-item ${ postStatus == 'any' ? 'status-active' : '' } `}>
                                            <div onClick={ () => { setPostStatus( 'any' ) ; setTotalRows ( subscribersStatus.totalSubscribers )  } } className="woo-multistatus-check-all ">
                                                {`All (${ subscribersStatus.totalSubscribers })`}
                                            </div>
                                        </li>
                                        <li  className="woo-multistatus-item woo-divider"></li>
                                        <li onClick={ () => { setPostStatus ( 'woo_subscribed' ) ; setTotalRows ( subscribersStatus.subscribed ) } } className={`woo-multistatus-item ${ postStatus == 'woo_subscribed' ? 'status-active' : '' } `}>
                                            <div className="woo-multistatus-check-subscribe">
                                                {`Subscribe (${ subscribersStatus.subscribed })`}
                                            </div>
                                        </li>
                                        <li  className="woo-multistatus-item woo-divider"></li>
                                        <li onClick={ () => { setPostStatus ( 'woo_unsubscribed' ) ; setTotalRows ( subscribersStatus.unsubscribed ) } } className={`woo-multistatus-item ${ postStatus == 'woo_unsubscribed' ? 'status-active' : '' } `}>
                                            <div className="woo-multistatus-check-unpaid">
                                                {`Unsubscribe (${ subscribersStatus.unsubscribed })`}
                                            </div>
                                        </li>
                                        <li className="woo-multistatus-item woo-divider"></li>
                                        <li onClick={ () => { setPostStatus ( 'woo_mailsent' ) ;setTotalRows ( subscribersStatus.mailSent ) } } className={`woo-multistatus-item ${ postStatus == 'woo_mailsent' ? 'status-active' : '' } `}>
                                            <div className="woo-multistatus-check-unpaid">
