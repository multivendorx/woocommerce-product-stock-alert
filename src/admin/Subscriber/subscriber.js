...
class Subscriber extends Component {
	...
	handleupdatesub(e) {
		...
		axios
		.get(
			`${stockManagerAppLocalizer.apiUrl}/woo-stockmanager-pro/v1/show-subscribe-from-status-list`,
			{
				params: { date_range: e },
			}
		).then((response) => {
    ...
    handlesubscriptionsearch(e, status) {
		 if (status === 'searchproduct') {
			if (e && e.target.value.length > 2) {
				axios
					.get(
						`${stockManagerAppLocalizer.apiUrl}/woo-stockmanager-pro/v1/search-subscribe-by-product`,
						{
    ...
    handle_subscription_live_search(e) {
		if (e.target.value) {
			axios
				.get(
					`${stockManagerAppLocalizer.apiUrl}/woo-stockmanager-pro/v1/search-specific-subscribe`,
					{
    ...
    common_funtions = (e) => {
		...
		axios
			.get(
				`${stockManagerAppLocalizer.apiUrl}/woo-stockmanager-pro/v1/no-of-subscribe-list`,
				{
