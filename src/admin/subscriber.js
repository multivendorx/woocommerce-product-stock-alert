...
class Subscriber extends Component {
	...
	handleupdatesub(e) {
		...
		axios
		.get(
			`${stockalertappLocalizer.apiUrl}/woo_stockalert_pro/v1/show_subscribe_from_status_list`,
			{
				params: { date_range: e },
			}
		).then((response) => {
    ...
    handlesubscriptionsearch(e, status) {
		 if (status === 'searchproduct') {
			if (e && e.target.value.length > 2) {
				axios
					.get(
						`${stockalertappLocalizer.apiUrl}/woo_stockalert_pro/v1/search_subscribe_by_product`,
						{
    ...
    handle_subscription_live_search(e) {
		if (e.target.value) {
			axios
				.get(
					`${stockalertappLocalizer.apiUrl}/woo_stockalert_pro/v1/search_specific_subscribe`,
					{
    ...
    common_funtions = (e) => {
		...
		axios
			.get(
				`${stockalertappLocalizer.apiUrl}/woo_stockalert_pro/v1/no_of_subscribe_list`,
				{
